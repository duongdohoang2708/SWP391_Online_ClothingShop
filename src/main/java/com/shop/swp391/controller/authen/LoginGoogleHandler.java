...
public class LoginGoogleHandler extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        User user = GlobalUtils.convertToAccount(userGoogleDto);
        
        try {
            //check  user exist by mail
            User userInDB = userDao.findByEmail(User.builder().email(user.getEmail()).build());
            //if not exist => insert
            if (userInDB == null) {
                userDao.insert(user);
            }
            HttpSession session = request.getSession();
            user = userDao.findByEmail(User.builder().email(user.getEmail()).build());
